...
//    val clusterName = None: Option[String]
    ...
    df.createClickhouseTable(db, tableName, "mock_date", Seq("name"), clusterName)

    // save data
    val res = df.saveToClickhouse("tmp1", "t1", (row) => java.sql.Date.valueOf("2000-12-01"), "mock_date", clusterName)
    assert(res.size == 1)
